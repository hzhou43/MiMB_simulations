...
void particle_swap(){
	...
	xa = L[add]*(rand()/double(RAND_MAX));
	ya = L[add]*(rand()/double(RAND_MAX));
	za = L[add]*(rand()/double(RAND_MAX));
...
void volume_exch(){
	...
	Vn = vo[0] + vo[1];
	vn[0] = vo[0] + (2.0*(rand()/double(RAND_MAX))-1.0)*dVmax;
...
void particle_disp(){
	...
	xd = x[p] + (2.0*(rand()/double(RAND_MAX))-1.0)*lmax;
	yd = y[p] + (2.0*(rand()/double(RAND_MAX))-1.0)*lmax;
	zd = z[p] + (2.0*(rand()/double(RAND_MAX))-1.0)*lmax;
