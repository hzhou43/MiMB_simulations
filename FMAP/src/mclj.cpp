...
double mcljrun(int N, double L, double T, double rc, double dr, int nCycles, int nEq, int fs, int bs, int Seed);
...
EM_JS(void, call_alert, (int x), {
  ...
  /*
  setTimeout(function() {
      document.getElementById("sim_prog").value = x;
    }, 0);
  setTimeout(function() {
  	document.getElementById("sim_ex").value = x;
    }, 0);
  //setTimeout(function() {
  	document.getElementById("canvas1").getContext('2d').fillText("+", x*5, 20);
  //}, 0 );
...
double mcljrun(int N, double L, double T, double rc, double dr, int nCycles, int nEq, int fs, int ts, int Seed){
  ...
  MCNVTBase *nvt=new MCNVTBase(N,L,T,true,pb,N,eb);
  tp->init();
  nvt->init_cube();
  nvt->setdr(dr);
