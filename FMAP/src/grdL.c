...
void xyz2pos(const int n, const double xf[], const double yf[], const double zf[], int pos[]){
...
void ind2ijk(const int n, const int loc[], int xoff[], int yoff[], int zoff[]){
...
void setlocwgt(const int n, int pos[], const int m, int loc[], double w[], int Ind[1000][10], double Wght[1000][10]){
	#pragma omp for schedule(static) nowait
	for (int i=0;i<n;i++){
		loc[i]=Ind[pos[i]][m];
		w[i]=Wght[pos[i]][m];
...
void mult2(const int n, const double w[], const double coef[], double wn[]){
	#pragma omp for schedule(static) nowait
	for (int i=0;i<n;i++){
		wn[i]=w[i]*coef[i];
	}
...
void pbc1n(const int n, const int x[], int px[], const int lc, const int off[], const int shift){
        #pragma omp for schedule(static) nowait
        for (int i=0;i<n;i++){
...
#endif
        if (nlattice==0){
		char flt[]="lattice.txt";
                nlattice=setIndWght(flt,1000,Ind,Wght);
        }
...
//xc=1-xf
void setcomp(const int n, const double xf[], double xc[]){
	#pragma omp for schedule(static) nowait
	for (int i=0;i<n;i++){
		xc[i]=1.0-xf[i];
#ifdef DEBUG
...
void mult3(const int n, const double xc[], const double yc[], const double zc[], double w[]){
	#pragma omp for schedule(static) nowait
	for (int i=0;i<n;i++){
		w[i]=xc[i]*yc[i]*zc[i];
	}
...
void grdL0(const int l, fftw_real* grid,\
	...
	int N=2*(l/2+1);
	for (i=0;i<n;i++){
		px=PBC(((xf[i]>0.5)?xi[i]:xi[i]+1),l);
		py=PBC(((yf[i]>0.5)?yi[i]:yi[i]+1),l);
		pz=PBC(((zf[i]>0.5)?zi[i]:zi[i]+1),l);
		int ijk=(px*l+py)*N+pz;
		grid[ijk]+=coef1[i];
	}
}
